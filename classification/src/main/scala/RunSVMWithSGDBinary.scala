...
object RunSVMWithSGDBinary {
  ...
  //二、训练阶段
  def trainEvaluation(trainData: RDD[LabeledPoint], validationData: RDD[LabeledPoint]):SVMModel={
    println("开始训练。。。。。")
    val (model,time) = trainModel(trainData,25,100,1)
    println("训练完成。。。。\n 所需时间："+time+"毫秒！")
  ...
  def parametersTunning(trainData: RDD[LabeledPoint], validationData: RDD[LabeledPoint]):SVMModel={
    println("-----评估numIterations参数使用1,3,5,15,25----------")
  ...
  def evaluateParameter(trainData: RDD[LabeledPoint], validationData: RDD[LabeledPoint], evaluateParameter: String, numIterationsArray: Array[Int], stepSizeArray: Array[Double], regParamArray: Array[Double])={
    ...
    Chart.plotBarLineChart("SVMWithSGDBinary evaluations "+evaluateParameter,evaluateParameter,"AUC",0.58,0.7,"Time",dataBarChart,dataLineChart)
  }

  def evaluateAllParameter(trainData: RDD[LabeledPoint], validationData: RDD[LabeledPoint], numIterationsArray: Array[Int], stepSizeArray: Array[Double], regParamArray: Array[Double])={
